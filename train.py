...
def train_prompt(args, train_dataset, model, tokenizer, verbalizer, template):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()
    ...
    train_dataloader = PromptDataLoader(dataset=train_dataset, template=template, tokenizer=tokenizer,
                                        verbalizer=verbalizer, max_seq_length=args.max_seq_length, shuffle=True,
...
def evaluate_prompt(args, model, tokenizer, verbalizaer, template, prefix=""):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_task_names = (args.task_name,)
    eval_outputs_dirs = (args.output_dir,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_dataloader = PromptDataLoader(dataset=eval_dataset, template=template, tokenizer=tokenizer,
                                            verbalizer=verbalizaer, max_seq_length=args.max_seq_length,
...
def main():
    ...
    parser.add_argument("--prompt", action="store_true", help="Do prompting.")
    parser.add_argument(
    ...
    # plm, tokenizer_prompt, model_cofig_prompt, WrapperClass = load_plm("bert", "bert-base-uncased")
    ...
    if args.do_train:
        ...
        if args.prompt:
            ...
            prompt_model = PromptForClassification(plm=model, template=mytemplate, verbalizer=myverbalizer)
            global_step, tr_loss = train_prompt(args, train_dataset, prompt_model, tokenizer, myverbalizer, mytemplate)
        else:
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            if args.prompt:
                result = evaluate_prompt(args, prompt_model, tokenizer, myverbalizer, mytemplate, prefix=prefix)
            else:
